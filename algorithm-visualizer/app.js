...
async function selectionSort() { 
  ...
  for (let i = 0; i < arr.length - 1; i++) { 
    ...
    if (minIndex !== i) {
      ...
      bar_values[i].textContent = arr[i];
      bar_values[minIndex].textContent = arr[minIndex];
    }
    
    bars[i].style.background = "green";
  }
  
  bars[arr.length - 1].style.background = "green";
  disableButtons(false);
}
...
async function insertionSort() { 
  ...
  for (let i = 1; i < arr.length; i++) { 
    ...
    while (j >= 0 && arr[j] > key) {
      ...
      arr[j + 1] = arr[j];
      bars[j + 1].style.height = arr[j + 1] * 3 + "px";
      bar_values[j + 1].textContent = arr[j + 1];
      await sleep(getSpeed());
    ...
    bar_values[j + 1].textContent = key;
    bars[i].style.background = "green";
    await sleep(getSpeed());
  }
...
async function heapSort() { 
  ...
  for (let i = Math.floor(n / 2) - 1; i >= 0; i--) {
    await heapify(arr, n, i, bars, bar_values);
  }
  ...
  for (let i = n - 1; i > 0; i--) {
    ...
    bars[i].style.height = arr[i] * 3 + "px";
    bar_values[0].textContent = arr[0];
    bar_values[i].textContent = arr[i];
    bars[i].style.background = "green";
    ...
    await heapify(arr, i, 0, bars, bar_values);
  }
...
async function heapify(arr, n, i, bars, bar_values) {
  let largest = i;
  const left = 2 * i + 1;
  const right = 2 * i + 2;
  ...
  if (largest !== i) {
    ...
    bars[largest].style.background = "yellow";
    
    [arr[i], arr[largest]] = [arr[largest], arr[i]];
    bars[i].style.height = arr[i] * 3 + "px";
    bars[largest].style.height = arr[largest] * 3 + "px";
    bar_values[i].textContent = arr[i];
    bar_values[largest].textContent = arr[largest];
    
    incrementStep();
    await sleep(getSpeed());
    
    bars[i].style.background = "steelblue";
    bars[largest].style.background = "steelblue";
    
    await heapify(arr, n, largest, bars, bar_values);
  }
}
